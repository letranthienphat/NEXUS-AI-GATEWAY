...
# Cấu hình đường dẫn các file dữ liệu
DATA_FILE = "data.json"
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def add_model_action(self):
        ...
        self.parent.config["models"][name] = path.strip('"').strip("'")
...
class ChatApp(ctk.CTk):
    def __init__(self):
        ...
        # --- THANH NHẬP LIỆU PHÍA DƯỚI (MÀU CHỮ TRẮNG, XÓA KHI FOCUS CHI TIẾT) ---
        ...
        # Bắt sự kiện focus chuột vào/ra ô nhập liệu để ẩn dòng chữ gợi ý lập tức
    ...
    def load_settings_data(self):
        ...
        self.config = {
            ...
            "models": {
                "Qwen2.5-1.5B": r"C:\Users\Phat\Documents\Phát\Python\AI\qwen2.5-1.5b-instruct-q3_k_m.gguf"
            },
            "languages": {
                "vi": {"name": "Tiếng Việt"},
                "en": {"name": "English"}
    ...
    def load_model_worker(self):
        ...
        try:
            ...
            # Cấu hình siêu tối ưu bộ nhớ cho các dòng máy RAM thấp (4GB)
            self.ai = Llama(
                model_path=model_path, 
                n_ctx=512,         # Thu hẹp ngữ cảnh để tiết kiệm lượng RAM khổng lồ
                n_threads=4,       # Khớp với luồng xử lý của CPU i7 đời cũ
                use_mmap=True,     # Ánh xạ bộ nhớ ảo SSD để giảm tải trực tiếp cho RAM vật lý
                use_mlock=False,   # Không giữ cố định trên RAM vật lý tránh đơ hệ thống
                verbose=False
            )
    ...
    def unload_model_worker(self):
        if self.ai is not None:
            try:
                # Ngắt kết nối các đối tượng C++ bên dưới llama-cpp
        ...
        # Kích hoạt dọn rác tầng sâu nhiều lần để ép Windows nhả RAM ngay
        gc.collect()
        gc.collect()
    ...
    def generate_title_with_ai(self, chat_id, cau_hoi, cau_tra_loi):
        ...
        prompt_dat_ten = [
            {"role": "system", "content": "Đặt một tiêu đề cực kỳ ngắn gọn (từ 2 đến 5 từ) bằng chính ngôn ngữ người dùng đang giao tiếp dựa trên ngữ cảnh được cung cấp. Không dùng dấu ngoặc kép."},
